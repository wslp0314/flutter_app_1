library lp_password_input_textfield;

import 'package:flutter/material.dart';
import 'package:flutter/services.dart';

enum LPPasswordInputTextFieldType { BOXES, SEGMENTED ,LINE }

class LPPasswordInputTextField extends StatefulWidget {
  final LPPasswordInputTextFieldType type; //格子样式
  ...
  const LPPasswordInputTextField({
    Key key,
    this.onChange,
    this.length = 6,
    this.controller,
    ...
    this.type = LPPasswordInputTextFieldType.BOXES,
    this.fillColor = Colors.white,
    this.backgroundColor = Colors.white,
  }) : super(key: key);

  @override
  _LPPasswordInputTextFieldState createState() =>
      _LPPasswordInputTextFieldState();
}

class _LPPasswordInputTextFieldState extends State<LPPasswordInputTextField> {
  List titles = [];
  Widget box() {
    ...
    for (var i = 0; i < widget.length; i++) {
      ...
      if (widget.type == LPPasswordInputTextFieldType.BOXES) {
        borderRadius = BorderRadius.circular(widget.borderRaiuds);
        border =
            Border.all(width: widget.borderWidth, color: widget.borderColor);
      } else if (widget.type == LPPasswordInputTextFieldType.LINE) {
        // BorderSide side =
        // BorderSide(width: widget.borderWidth, color: widget.borderColor);
        // BorderSide sideNull = BorderSide(width: 0, color: widget.borderColor);
        //
        // border = Border(left: sideNull, bottom: side, top: sideNull, right: sideNull);
      } else if (widget.type == LPPasswordInputTextFieldType.SEGMENTED) {
        BorderSide side =
            BorderSide(width: widget.borderWidth, color: widget.borderColor);
      ...
      Widget item = Container(
        ...
        child: Column(
          children: <Widget>[
            ...
            Divider(height: 10.0,indent: itemWidth,color: Colors.red,),
          ],
        )
          ...
          Positioned(
            ...
            child: Container(
              ...
              child: TextField(
                ...
                inputFormatters: [WhitelistingTextInputFormatter.digitsOnly,LengthLimitingTextInputFormatter(widget.length)],
          ...
          GestureDetector(
            ...
            child: Container(
                ...
                child: Container(
                    ...
                    child: widget.type == LPPasswordInputTextFieldType.BOXES
                        ? Wrap(
